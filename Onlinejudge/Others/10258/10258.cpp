...
struct UserStatus{
	...
	void calculatePenalty(int time) {
		penalty = (attempts * 20) + time;
	}
};
...
void calculateScoreboard(int numberProblems) {
	...
	for(ProblemStatus problemStatus: problemEntries) {
		if(userScoreboard[problemStatus.contestant][problemStatus.problem].status != 'C') {
			if(problemStatus.status == 'I') {
				userScoreboard[problemStatus.contestant][problemStatus.problem].attempts++;
			} else if(problemStatus.status == 'C') {
				userScoreboard[problemStatus.contestant][problemStatus.problem].status = problemStatus.status;
				userScoreboard[problemStatus.contestant][problemStatus.problem].calculatePenalty(problemStatus.time);
			}	
			userScoreboard[problemStatus.contestant][problemStatus.problem].status = problemStatus.status;
		}
	}
	for(int i = 0; i < 101; i++) {
		...
		for(int j = 0; j < 11; j++){
			...
			if(userScoreboard[i][j].status == 'C'){
				...
				participation = true;
			} else if(userScoreboard[i][j].status == 'I' || userScoreboard[i][j].status == 'E' || userScoreboard[i][j].status == 'R' || userScoreboard[i][j].status == 'U') {
				participation = true;
			}
		}
		if(participation) {
...
void readInput() {
	...
	while (n--) {
		...
		while(fgets(input, sizeof(input), stdin) != NULL) {
			...
			sscanf(input, "%d%d%d %c", &problemStatus.contestant, &problemStatus.problem,&problemStatus.time, &problemStatus.status);
